...
// JSX host (DOM) elements are lowercase; capitalized names are components.
const isHostElement = (name) => /^[a-z]/.test(name);

// Build a rule that visits every JSX element whose tag name passes `matches`,
// and reports `messageId` on the ones that `violates`.
function defineRule({ description, messageId, message, matches, violates }) {
    return {
        meta: {
            type: 'problem',
            docs: { description, category: 'Possible Errors' },
...
const rules = {
    'non-void-dom-elements-self-closing': defineRule({
        description:
            'Disallow the self-closing style on non-void DOM elements (use <div></div> instead of <div />).',
        messageId: 'noSelfClose',
        message:
            'Skrapa: Non-void DOM element <{{ name }}> cannot be self-closing; use an explicit closing tag <{{ name }}></{{ name }}>.',
        // A non-void element self-closed (e.g. <div />). In HTML the self-closing
        // slash is ignored, so it must use a closing tag.
        matches: (name) => isHostElement(name) && !VOID_ELEMENTS.has(name),
        violates: (node) => node.openingElement.selfClosing,
    }),
    'void-dom-elements-no-children': defineRule({
        description:
            'Disallow void DOM elements from receiving children in non-React JSX contexts.',
        messageId: 'noChildrenInVoid',
        message: 'Skrapa: Void DOM element <{{ name }}> cannot have children or content.',
        matches: (name) => VOID_ELEMENTS.has(name),
        // Nested JSX children / text nodes, or an explicit `children` prop.
        violates: (node) =>
    ...
    'void-dom-elements-self-closing': defineRule({
        description:
            'Require void DOM elements to use the self-closing style (<hr /> instead of <hr></hr>).',
        messageId: 'mustSelfClose',
        message:
            'Skrapa: Void DOM element <{{ name }}> must use the self-closing style <{{ name }} />.',
        // Written with an explicit closing tag (e.g. <hr></hr>) instead of <hr />.
        matches: (name) => VOID_ELEMENTS.has(name),
        violates: (node) => !node.openingElement.selfClosing,
    }),
};
